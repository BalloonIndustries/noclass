 ...
 */
global $theme_key, $path_to_emu;
$theme_key = $GLOBALS['theme_key'];
$path_to_emu = drupal_get_path('theme', 'emu');
include_once($path_to_emu . '/inc/load.inc');       // drupal_add_css() wrappers
include_once($path_to_emu . '/inc/get.inc');        // get theme info, settings, css etc

/**
 * Construct HTML code for the top/bottom block columns
 */
function emu_build_columns($columns) {
  $styles = array();
  $num_columns = 0;
  $first = -1;
 ...
 */
function emu_breadcrumb($variables) {
 ...
 */
function emu_preprocess_search_block_form(&$variables) {
 ...
 */
function emu_process_page(&$variables) {
 ...
 */
function emu_preprocess_maintenance_page(&$variables) {
  drupal_add_css(drupal_get_path('theme', 'emu') . '/css/maintenance-page.css');
}

/**
 * Implements hook_preprocess_html().
 */
function emu_preprocess_html(&$variables) {
  global $theme_key;
  ...
  if (theme_get_setting('menubar_style') == 2) {
    drupal_add_css(drupal_get_path('theme', 'emu') . '/css/black-menu.css', $options);
  }

  // Add old legacy MAYO styles for Superfish.
  if (theme_get_setting('legacy_superfish_styles') == 1) {
    drupal_add_css(drupal_get_path('theme', 'emu') . '/css/emu-superfish.css', $options);
  }
  ...
  $path = variable_get('theme_' . $theme_name . '_files_directory');
    // Load Layout
  emu_load_layout_css($path, $theme_name);
}

/**
 * Implements hook_process_html().
 */
function emu_process_html(&$variables) {
  // Hook into color.module
  if (module_exists('color')) {

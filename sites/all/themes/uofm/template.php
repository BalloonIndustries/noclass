 ...
 */
global $theme_key, $path_to_uofm;
$theme_key = $GLOBALS['theme_key'];
$path_to_uofm = drupal_get_path('theme', 'uofm');
include_once($path_to_uofm . '/inc/load.inc');       // drupal_add_css() wrappers
include_once($path_to_uofm . '/inc/get.inc');        // get theme info, settings, css etc

/**
 * Construct HTML code for the top/bottom block columns
 */
function uofm_build_columns($columns) {
  $styles = array();
  $num_columns = 0;
  $first = -1;
 ...
 */
function uofm_breadcrumb($variables) {
 ...
 */
function uofm_preprocess_search_block_form(&$variables) {
 ...
 */
function uofm_process_page(&$variables) {
 ...
 */
function uofm_preprocess_maintenance_page(&$variables) {
  drupal_add_css(drupal_get_path('theme', 'uofm') . '/css/maintenance-page.css');
}
...
function uofm_preprocess_html(&$variables) {
  ...
  if (theme_get_setting('menubar_style') == 2) {
    drupal_add_css(drupal_get_path('theme', 'uofm') . '/css/black-menu.css', $options);
  }

  // Add old legacy MAYO styles for Superfish.
  if (theme_get_setting('legacy_superfish_styles') == 1) {
    drupal_add_css(drupal_get_path('theme', 'uofm') . '/css/uofm-superfish.css', $options);
  }
  ...
  $path = variable_get('theme_' . $theme_name . '_files_directory');
    // Load Layout
  uofm_load_layout_css($path, $theme_name);
}

/**
 * Implements hook_process_html().
 */
function uofm_process_html(&$variables) {
  // Hook into color.module
  if (module_exists('color')) {

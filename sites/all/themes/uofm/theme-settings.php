 ...
 */
function uofm_form_system_theme_settings_alter(&$form, &$form_state) {

  drupal_add_js(drupal_get_path('theme', 'uofm') . '/js/uofm.js');
    // Get our plugin system functions.
  require_once(drupal_get_path('theme', 'uofm') . '/inc/plugins.inc');

  // We need some getters.
  require_once(drupal_get_path('theme', 'uofm') . '/inc/get.inc');
  $path_to_uofm = drupal_get_path('theme', 'uofm');
  ...
  $device_group_options = page_layouts_device_group_options('uofm');
  ...
  $form['layout']['base_vmargin'] = array(
    ...
    '#prefix' => '<img src="/' . drupal_get_path('theme', 'uofm') . '/images/base-layout.png" /><br />',
  );
  ...
  $form['layout']['layout_style'] = array(
    ...
    '#description' => '<img src="/' . drupal_get_path('theme', 'uofm') . '/images/page-layout.png" /><br />' . t('When the layout 2 is selected, or header background image is selected, header borders are not drawn to make it look better.'),
  );
  ...
  $form['style']['legacy_superfish_styles'] = array(
    ...
    '#description' => t('Check here if you want to add uofm-superfish.css.'),
  );
  ...
  $form['style']['round_corners'] = array(
    ...
    '#suffix' => '<img src="/' . drupal_get_path('theme', 'uofm') . '/images/round-corners.png" /><br />',
  );
  ...
  $form['style']['menubar_style'] = array(
    ...
    '#suffix' => '<img src="/' . drupal_get_path('theme', 'uofm') . '/images/menubar-type.png" />',
  );
  ...
  $form['adv_header']['header_height'] = array(
    ...
    '#prefix' => '<img src="/' . drupal_get_path('theme', 'uofm') . '/images/header-layout.png" /><br />',
  );
  ...
  $form['adv_header']['header_bg'] = array(
    ...
    '#element_validate' => array('uofm_header_bg_validate'),
  );
  ...
  $form['adv_header']['header_watermark'] = array(
    ...
    '#suffix' => '<img src="/' . drupal_get_path('theme', 'uofm') . '/images/watermark-sample.png" /><br />',
  );
  ...
  require_once(drupal_get_path('theme', 'uofm') . '/inc/forms/uofm.validate.inc');
  require_once(drupal_get_path('theme', 'uofm') . '/inc/forms/uofm.submit.inc');

  // Custom validate and submit functions
  $form['#validate'][] = 'uofm_settings_validate';
  $form['#submit'][] = 'uofm_settings_submit';
}

/**
 * Check and save the uploaded header background image
 */
function uofm_header_bg_validate($element, &$form_state) {
  global $base_url;
 ...
 * Validate page width
 */
function uofm_page_width_validate($element, &$form_state) {
 ...
 */
function uofm_sidebar_width_validate($element, &$form_state) {
  if (!empty($element['#value'])) {
    $width = $element['#value'];
